...
updateGameEvents()
{
	maps\mp\gametypes\mm::debug("updateGameEvents() -> called");
	if ( matchMakingGame() && !level.inGracePeriod )
	{
		maps\mp\gametypes\mm::debug("updateGameEvents() -> !inGracePeriod");
		if ( level.teamBased )
		{
	...
	maps\mp\gametypes\mm::debug("updateGameEvents() -> !level.disableSpawning = " + !level.disableSpawning);

	if ( !getGametypeNumLives() && (!isDefined( level.disableSpawning ) || !level.disableSpawning) )
		return;
	
	maps\mp\gametypes\mm::debug("updateGameEvents() -> !gameHasStarted() = " + !gameHasStarted());

	if ( !gameHasStarted() ) 
		return;
	
	maps\mp\gametypes\mm::debug("updateGameEvents() -> !level.inGracePeriod = " + !level.inGracePeriod);
	...
	if ( level.teamBased )
	{
		maps\mp\gametypes\mm::debug("updateGameEvents() -> teamBased");
		livesCount["allies"] = level.livesCount["allies"];
		livesCount["axis"] = level.livesCount["axis"];

		maps\mp\gametypes\mm::debug("updateGameEvents() -> alivesCount[allies] = " + livesCount["allies"]);
		maps\mp\gametypes\mm::debug("updateGameEvents() -> alivesCount[axis] = " + livesCount["axis"]);
		...
		maps\mp\gametypes\mm::debug("updateGameEvents() -> one ally left: " + level.aliveCount["allies"] == 1 && !livesCount["allies"]);
		...
		maps\mp\gametypes\mm::debug("updateGameEvents() -> one axis left: " + level.aliveCount["axis"] == 1 && !livesCount["axis"]);
